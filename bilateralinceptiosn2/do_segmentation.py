...
def do_segmentation(prototxt, caffe_model, image_dir, 
    ...
    #output_path = '/home/fil/Documentos/bilateralinceptions2'
    ...
    with open(image_list) as list_f:
        for count, imgname in enumerate(list_f):
            ...
            spixel_index = np.expand_dims(np.expand_dims(spixel_index, axis=0),
                                          axis = 0)
            im2 = im
            result = net.forward_all(img=im, spixel_index=spixel_index, img2=im)
            #print result.shape
            #for out in result:
                #np.savetxt('test.out', out)
            ...
            np.savetxt('result_prediction.txt', np.uint8(seg_map) ) 
            print seg_map.shape
    ...
    #for layer_name, blob in net.blobs.items():
        ...
        #idx = [i for i in range(len(net._layer_names)) if(net._layer_names[i] == layer_name)]
        #if(len(idx) == 0):
        #    continue
        #layer_type = net.layers[idx[0]].type
