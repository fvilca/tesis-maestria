	...
	)
{
	...
	//line(canvas,Point(x,y),Point(img_x,img_y),color,1,8,0);
	circle(canvas, Point(img_x,img_y),3, color,1, cv::LINE_8, 0);
	// TODO: go one step towards gradients direction

	cout<<"idx: "<<cluster_idx<<endl;
	out_spixel[cluster_idx].id = cluster_idx;
	out_spixel[cluster_idx].center = gSLICr::Vector2f((float)img_x, (float)img_y);
	out_spixel[cluster_idx].color_info = inimg[img_y*img_size.x + img_x];
	out_spixel[cluster_idx].no_pixels = 0;
	...
	// fout<<setw(3)<<x<<","<<y<<" - ";
	// fout<<setw(5)<<out_spixel[cluster_idx].id<<" => ";
	// fout<<" center: "<<setw(3)<<out_spixel[cluster_idx].center.x<<","<<setw(3)<<out_spixel[cluster_idx].center.y;
	// fout<<" color: ";
	// fout<<endl;
...
//TODO
inline void find_center_association_shared2(
	const gSLICr::Vector4f* inimg, 
	const gSLICr::objects::spixel_info* in_spixel_map, 
	int* out_idx_img, 
	gSLICr::Vector2i map_size, gSLICr::Vector2i img_size, int spixel_size, float weight, 
	int x, int y, float max_xy_dist, float max_color_dist/*,  ostream& fout*/
	)
{
	int idx_img = y * img_size.x + x;
	//fout<<idx_img<<","<<" "<<",";
...
_CPU_AND_GPU_CODE_ inline void draw_superpixel_boundry_shared(const int* idx_img, gSLICr::Vector4u* sourceimg, gSLICr::Vector4u* outimg, gSLICr::Vector2i img_size, int x, int y)
{
 ...
 _CPU_AND_GPU_CODE_ inline void merge_shared(int* idx_img, gSLICr::Vector2i img_size, int x, int y)
{
	int idx = y * img_size.x + x;

	if (idx_img[idx] >= 13 && idx_img[idx] <= 25)
	{
		idx_img[idx] = 12;
	}

	if ( (idx_img[idx] >= 192 && idx_img[idx] <= 197) )
	{
		idx_img[idx] = 192;
	}

	if ( (idx_img[idx] >= 228 && idx_img[idx] <= 230) || (idx_img[idx] >= 264 && idx_img[idx] <= 266)  )
	{
		idx_img[idx] = 228;
	}

	if ( (idx_img[idx] >= 231 && idx_img[idx] <= 233) || (idx_img[idx] >= 267 && idx_img[idx] <= 269)  )
	{
		idx_img[idx] = 231;
	}

	if ( (idx_img[idx] >= 300 && idx_img[idx] <= 302) || (idx_img[idx] >= 336 && idx_img[idx] <= 338)  )
	{
		idx_img[idx] = 300;
	}
	if ( (idx_img[idx] >= 303 && idx_img[idx] <= 305) || (idx_img[idx] >= 339 && idx_img[idx] <= 341)  )
	{
		idx_img[idx] = 303;
	}
	///
	if ( (idx_img[idx] >= 306 && idx_img[idx] <= 312) || (idx_img[idx] >= 342 && idx_img[idx] <= 348)  )
	{
		idx_img[idx] = 306;
	}
	//*****************
	if ( (idx_img[idx] >= 234 && idx_img[idx] <= 240) || (idx_img[idx] >= 270 && idx_img[idx] <= 276)  )
	{
		idx_img[idx] = 234;
	}
