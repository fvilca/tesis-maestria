...
__global__ void Init_Cluster_Centers_device(const Vector4f* inimg, spixel_info* out_spixel, Vector2i map_size, Vector2i img_size, int spixel_size);
		   ...
		   void Init_Cluster_Centers_device2(const Vector4f* inimg, spixel_info* out_spixel, Vector2i map_size, Vector2i img_size, int spixel_size);
...
__global__ void Find_Center_Association_device(const Vector4f* inimg, const spixel_info* in_spixel_map, int* out_idx_img, Vector2i map_size, Vector2i img_size, int spixel_size, float weight, float max_xy_dist, float max_color_dist);
           ...
           void Find_Center_Association_device2(const Vector4f* inimg, const spixel_info* in_spixel_map, int* out_idx_img, Vector2i map_size, Vector2i img_size, int spixel_size, float weight, float max_xy_dist, float max_color_dist);
...
__global__ void merge_device( int* idx_img, Vector2i img_size);
...
seg_engine_GPU::seg_engine_GPU(const settings& in_settings) : seg_engine(in_settings)
{
	...
	cout<<"spixel_size: "<<spixel_size<<endl;
...
void gSLICr::engines::seg_engine_GPU::Init_Cluster_Centers2()
{
	/*spixel_info* spixel_list = spixel_map->GetData(MEMORYDEVICE_CUDA);
	Vector4f* img_ptr = cvt_img->GetData(MEMORYDEVICE_CUDA);*/
	...
	cout<<"(float)blockSize: "<<(float)blockSize.x<<","<<(float)blockSize.y<<endl;
	cout<<"(float)gridSize:  "<<(float)gridSize.x<<","<<(float)gridSize.y<<endl;

	Init_Cluster_Centers_device2 (img_ptr, spixel_list, map_size, img_size, spixel_size);
}
...
void gSLICr::engines::seg_engine_GPU::Find_Center_Association2()
{
	 spixel_info* spixel_list = spixel_map->GetData(MEMORYDEVICE_CPU);
	 Vector4f* img_ptr = cvt_img->GetData(MEMORYDEVICE_CPU);
	...
	// dim3 gridSize((int)ceil((float)img_size.x / (float)blockSize.x), (int)ceil((float)img_size.y / (float)blockSize.y));
	 ...
	 Find_Center_Association_device2(
		 img_ptr, 
		 spixel_list, 
		 idx_ptr, 
		 map_size, 
...
void gSLICr::engines::seg_engine_GPU::Update_Cluster_Center2()
{
	// spixel_info* accum_map_ptr = accum_map->GetData(MEMORYDEVICE_CUDA);
	// spixel_info* spixel_list_ptr = spixel_map->GetData(MEMORYDEVICE_CUDA);
	// Vector4f* img_ptr = cvt_img->GetData(MEMORYDEVICE_CUDA);
	...
	spixel_info* accum_map_ptr = accum_map->GetData(MEMORYDEVICE_CPU);
	spixel_info* spixel_list_ptr = spixel_map->GetData(MEMORYDEVICE_CPU);
	Vector4f* img_ptr = cvt_img->GetData(MEMORYDEVICE_CPU);
	int* idx_ptr = idx_img->GetData(MEMORYDEVICE_CPU);
...
void gSLICr::engines::seg_engine_GPU::Draw_Segmentation_Result(UChar4Image* out_img)
{
	...
	Vector2i img_size = idx_img->noDims;
	//cout<< "idx_img->noDims: " << idx_img->noDims << endl;
...
void gSLICr::engines::seg_engine_GPU::merge(){
	...
	merge_device<<<gridSize,blockSize>>>(idx_img_ptr, img_size);
}
...
__global__ void merge_device(int* idx_img, Vector2i img_size)
{
	...
	merge_shared(idx_img, img_size, x, y);
}
...
{
	int x = threadIdx.x + blockIdx.x * blockDim.x, y = threadIdx.y + blockIdx.y * blockDim.y;
	if (x > map_size.x - 1 || y > map_size.y - 1) return; //con esto garantiza que no se salga del tamaño del mapa
	// ps hay un nro mayor de treads que el tamaño del mapa
...
void Init_Cluster_Centers_device2(const Vector4f* inimg, spixel_info* out_spixel, Vector2i map_size, Vector2i img_size, int spixel_size)
{
	...
	cout<<"img_size(x,y): "<<img_size.x<<","<<img_size.y<<endl;
	cout<<"map_size(x,y): "<<map_size.x<<","<<map_size.y<<endl;

	if (x > map_size.x - 1 || y > map_size.y - 1){ 
		cout<<"if (x > map_size.x - 1 || y > map_size.y - 1)"<<endl;
		return;
	}

	//ToDo
	
	Mat canvas(150,147,CV_8UC3,Scalar(0,0,0));
	...
	for(x = 0;x < 37;x++)
	{
		for(y = 0;y < 36;y++)
		{
			init_cluster_centers_shared2(inimg, out_spixel, map_size, img_size, spixel_size, x, y,fout, canvas, rng);		
		}
		//fout<<endl;
	}
...
void Find_Center_Association_device2(const Vector4f* inimg, const spixel_info* in_spixel_map, int* out_idx_img, Vector2i map_size, Vector2i img_size, int spixel_size, float weight, float max_xy_dist, float max_color_dist)
{
	...
	for(x = 0;x < 150;x++)
	{
		...
		{
			//fout<<x<<","<<y<<",";
			find_center_association_shared2(
			inimg, in_spixel_map, out_idx_img, 
			map_size, img_size, spixel_size, 
			weight, x, y, max_xy_dist, max_color_dist
			/*,fout*/);
			//fout<<endl;
		}
	}
