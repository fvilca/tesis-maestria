...
void save_data(int* idx_ptr){

	
	int x,y;
	ofstream fout("out_idx_img_final.txt");
	for(x = 0;x < 150;x++)
	{
	 	for(y = 0;y < 147;y++)
	 	{
	 		int idx_img = y * 150 + x;
	 		fout<<idx_ptr[idx_img]<<",";
	 	}
	 	fout<<endl;
	 }
...
void seg_engine::Perform_Segmentation2(UChar4Image* in_img)
{
	source_img->SetFrom(in_img, ORUtils::MemoryBlock<Vector4u>::CPU_TO_CPU);
	Cvt_Img_Space(source_img, cvt_img, gSLICr_settings.color_space);

	Init_Cluster_Centers2();
	Find_Center_Association2();

	for (int i = 0; i < gSLICr_settings.no_iters; i++)
	{
		Update_Cluster_Center2();
		Find_Center_Association2();
	}
