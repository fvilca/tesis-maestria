...
int main()
{
    ...
    //caffe::ReadProtoFromTextFile("solver.prototxt", &solver_param);
...
float *label;
/////////////////////////////////////////////////////////////////
void generate_tranning_data(){
	data = new float[64*1*1*3*400];
	label = new float[64*1*1*1*400];

	for (int i = 0; i < 64*1*1*400; ++i)
	{
		int a = rand() % 2;
	  	int b = rand() % 2;
	  	int c = a ^ b; //xor binary operator
	  	data[i*2+0] = a;
	  	data[i*2+1] = b;
	  	label[i] = c;	
	}
